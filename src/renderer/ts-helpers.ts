...
export function getNodeLabel(node: ts.Node) { return tsGetSyntaxTypeLabel(node.kind) ?? 'Unknown'; };

export const enum tsTypeMapKind {
    Simple,
    Array,
    Deferred,
    Function,
...
export function tsGetMappedType(type: ts.Type, mapper: tsTypeMapper, tc: ts.TypeChecker): ts.Type {
  switch (mapper.kind) {
...
export function tsGetSourceFileOfNode(node: ts.Node | undefined): ts.SourceFile | undefined {
...
export function tsGetSyntaxTypeLabel(kind: ts.SyntaxKind) {
