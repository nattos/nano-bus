import { PathPoint } from "./interconnect-layout";

export type Point = [x: number, y: number];

export function getPathPointXY(point: PathPoint): Point {
  const y = point.lane.y + point.laneLocalY;
  const x = point.x + point.xLocalX + 0.5;
  return [x, y];
}
